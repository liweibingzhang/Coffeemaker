...
public class APIRecipeController extends APIController {
    ...
    public ResponseEntity createRecipe ( @RequestBody final Recipe recipe ) {
        // The following print statements help log calls to the createRecipe
        // endpoint, and show the ingredients passed in.
        // System.out.println( "We got a recipe. It's name is" );
        // System.out.println( recipe.getName() );
        // System.out.print( "It has this many ingredients" );
        // System.out.println( recipe.getIngredients().size() );
        // System.out.println( recipe.getIngredients().get( 0 ).getName() +
        // recipe.getIngredients().get( 0 ).getAmount() );
        // System.out.println( recipe.getIngredients().get( 1 ).getName() +
        // recipe.getIngredients().get( 1 ).getAmount() );
    ...
    public ResponseEntity updateRecipe ( @RequestBody final Recipe other ) {
        final Recipe recipe = service.findByName( other.getName() );
        if ( recipe == null ) {
            return new ResponseEntity( errorResponse( "No recipe found with name " + other.getName() ),
                    HttpStatus.NOT_FOUND );
        }

        recipe.updateRecipe( other );
        service.save( recipe );
        return new ResponseEntity( successResponse( other.getName() + " was updated successfully" ), HttpStatus.OK );
    }
}
